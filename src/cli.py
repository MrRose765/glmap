...
from mapping.event_mapper import EventMapper
...
def main():
    ...
    # Internal paths
    processed_folder = os.path.join("data", "processed-events")
    ...
    try:
        ...
        for event_file in processed_event_files:
            ...
            mapped_actions = [event_mapper.map_event_to_action(event) for event in event_records]
        ...
        action_mapper = ActionMapper(actions, activity_mapping)

        activities = action_mapper.map()
        save_to_jsonl_file(activities, args.output_activities)
        print(f"Step 2 completed. Activities saved to: {args.output_activities}")

    finally:
        # Cleanup: Remove the processed folder
        if os.path.exists(processed_folder):
            shutil.rmtree(processed_folder)
            print(f"Cleanup completed. Removed intermediate folder: {processed_folder}")
